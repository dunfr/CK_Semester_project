...
`feat/battle-core`는 `prototype`에서 분기한 전투 로직의 첫 단계다. Unity 씬이나 UI 없이 실행 가능한 C# 코어와 EditMode 테스트를 제공한다.
...
이번 구현의 검증 결과: Unity 6000.3.23f1 EditMode 26개 통과, 실패·건너뜀 0개. 같은 테스트를 별도 .NET 실행기에서도 실행해 26개 통과했다. 씬·UI·연출 연결은 이 테스트의 검증 범위에 포함되지 않는다.
